...
#define EDMA_V0_DOORBELL_STOP				BIT(31)
...
enum dma_status_t {
	DX_LIE_STS_IDLE  = 0x00,
	DX_LIE_STS_START = 0x01,
	DX_LIE_STS_RUNN  = 0x02,
	DX_LIE_STS_DONE  = 0x03,
};
struct npu_sw_reg_t {
    volatile unsigned int dma_rd_sts;
} __packed;

struct dw_npu_sw_reg_t {
	u32 padding[7];
	struct npu_sw_reg_t reg[4];
} __packed;
