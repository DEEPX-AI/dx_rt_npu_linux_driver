...
{
    volatile void __iomem *reg = npu->reg_ctrl;
    pr_info( "%s\n", __func__);

    WRITE_CONTROL_NONSEC(reg, 0x3);//NONSEC[2:1]    
    WRITE_CONTROL_RESET(reg, 0xF); //APB[3] CORE[2] AXI[1] AXI_BRG[0] Reset Release    
    WRITE_CONTROL_CLOCK(reg, 0xF); //APB[3] CORE[1] AXI[1] AXI_BRG[0] Clock Release
}
...
static void get_ppu_data(dxnpu_t *npu)
{
    ...
    if(filter_num>0)
    {
        ...
        WRITE_DMA_USR_MODE_EN(reg, 1);//WRITE_DMA_DMA_CTRL(reg, 2);
        WRITE_DMA_USR_DMA_AXI_WADDR(reg, READ_SYSTEM_SWREG2(reg));
        WRITE_DMA_USR_DMA_SRAM_RADDR(reg, 0x04008800);
        WRITE_DMA_USR_DMA_WSIZE(reg, 128*1024);
        WRITE_DMA_USR_DMA_CMD(reg, 2);
        
        // poll dma irq
        pr_debug("%s: poll dma irq\n", __func__);
        // mdelay(500); // temp.
        while( (READ_DMA_USR_DMA_STATUS(reg)&2) != 2);

        // clear dma irq
        pr_debug("%s: clear dma irq\n", __func__);
        WRITE_DMA_USR_DMA_STATUS(reg, 2);
		WRITE_DMA_USR_MODE_EN(reg, 0);//added
...
static irqreturn_t npu_irq_handler(int irq, void *data)
{
    ...
    if(model_type == MODEL_TYPE_PPU)    get_ppu_data(npu);
    ...
    response->inf_time = (uint32_t)((uint64_t)READ_PROFILER_LAYER_PROFILER1_CLK_CNT(reg) * 1000 / npu->clock_khz);
    WRITE_PROFILER_LAYER_PROFILER0_LAYER_PROFILE_EN(reg, 0);
    ...
    WRITE_SYSTEM_SWREG1(reg, 0x0);//npu unlocked
...
int dx_v3_npu_init(dxnpu_t *npu)
{
    ...
    npu->reg_ctrl = ioremap(npu->reg_ctrl_addr, 0x1000);
    if(!npu->reg_ctrl)
    {
        pr_err("Failed to map npu ctrl registers\n");
        return -ENOMEM;
    }
    dx_v3_npu_sys_enable(npu);
...
{    
	...
	//pr_debug("    npu%d (DX-V3) @ %x: IRQ: %d, ID: %X, MODE: %X, AXI_CFG: %X\n",
...
int dx_v3_npu_run(dxnpu_t *npu, void *data)
{
    ...
    WRITE_SYSTEM_SWREG1(reg, 0xFFAA);//npu locked
    ...
    WRITE_SYSTEM_SWREG3(reg, req->model_type);//ppu => 2, non-ppu => 1
    //WRITE_SYSTEM_SWREG1(reg, 0xFFAA);
    WRITE_PROFILER_LAYER_PROFILER0_LAYER_PROFILE_EN(reg, 1);//WRITE_DEBUG_STAMP0_STAMP_EN(reg, 1);
