...
struct xdma_cdev *g_xdma_h2c_cdev[16] = {};
struct xdma_cdev *g_xdma_c2h_cdev[16] = {};
...
static u32 npu_num = 1;
module_param(npu_num, uint, 0644);
MODULE_PARM_DESC(npu_num, "the number of NPU (default:1, max:3)");
...
static int config_kobject(struct xdma_cdev *xcdev, enum cdev_type type, int cnt)
{
	int rv = -EINVAL;
...
static int create_sys_device(struct xdma_cdev *xcdev, enum cdev_type type, int cnt)
{
	struct xdma_dev *xdev = xcdev->xdev;
	struct xdma_engine *engine = xcdev->engine;
	int last_param;
			...
			enum cdev_type type, int cnt)
{
	...
	xcdev->bar = bar;
	xcdev->id = xdev->idx;
	if(xcdev->id >=0 && xcdev->id < 16)
	{		
		if(type==CHAR_XDMA_H2C)
		{
			pr_info("%s\n", __func__);
			g_xdma_h2c_cdev[xcdev->id] = xcdev;
		}
		if(type==CHAR_XDMA_C2H)
		{
			pr_info("%s\n", __func__);
			g_xdma_c2h_cdev[xcdev->id] = xcdev;
		}
	}

	rv = config_kobject(xcdev, type, cnt);
	if (rv < 0)
		return rv;
...
int xpdev_create_interfaces(struct xdma_pci_dev *xpdev)
{
	...
	if (npu_num > USER_CDEV_NUM)
		npu_num = USER_CDEV_NUM;
	...
	if (xdev->bypass_bar_idx > 0) {
		...
		rv = create_xcdev(xpdev, &xpdev->bypass_cdev_base,
				xdev->bypass_bar_idx, NULL, CHAR_BYPASS, 0);
		if (rv < 0) {
	...
	if (xdev->user_bar_idx >= 0) {
		for(i = 0; i < npu_num; i++) {
			rv = create_xcdev(xpdev, &xpdev->user_cdev[i], xdev->user_bar_idx,
				NULL, CHAR_USER, i);
			if (rv < 0) {
