...
// API to dxrt driver
extern struct xdma_cdev *g_xdma_h2c_cdev[], *g_xdma_c2h_cdev[];
ssize_t api_char_sgdma_read_write(int id, const char __user *buf,
		size_t count, loff_t *pos, bool write)
{

	int rv;
	ssize_t res = 0;	
	struct xdma_cdev *xcdev = write?g_xdma_h2c_cdev[id]:g_xdma_c2h_cdev[id];
	struct xdma_dev *xdev;
	struct xdma_engine *engine;
	struct xdma_io_cb cb;
	...
	dbg_tfr("xcdev 0x%p, buf 0x%p,%llu, pos %llu, W %d, %s.\n",
		...
		engine->name);
	// pr_info("xcdev 0x%p, buf 0x%p,%llu, pos %llu, W %d, %s.\n",
	// 	xcdev, buf, (u64)count, (u64)*pos, write,
	// 	engine->name);
